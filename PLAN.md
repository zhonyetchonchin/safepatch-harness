...
- [x] T63 WebUI 工作台
  ...
  - 验证：`pytest tests/api/test_webui_contract.py` 通过，`2 passed`；`pytest tests/api -q` 通过，`8 passed`；全量 `pytest -q` 通过，`79 passed, 1 skipped`。后续已通过 Render 公网部署 `https://safepatch-harness.onrender.com`，线上验证 WebUI、run、凭据接口正常。
  - 依赖：T60、T61、T62。
  - commit：`beb5dcc`。
...
- [x] T73 最终交付检查
  ...
  - 验证：`pytest tests/distribution/test_final_delivery.py` 通过，`1 passed`；全量 `pytest -q` 最终通过，`104 passed, 1 skipped`；REFLECTION.md 已由学生本人完成。
  - 依赖：全部实现 task。
  - commit：`7d3bd0d`。
...
- [x] T81 多轮 loop 与安全边界加固
  ...
  - 验证：多步脚本 MockLLM 先返回失败检查再改变动作并最终 finish；loop/budget/feedback/hitl/demo 测试全部通过。
  - 依赖：T80。
  - commit：`adc65fe`。

- [x] T82 Run 持久化与 Web demo 编排
  - 目标：runs 落 SQLite；四个安全 mock 场景通过真实 loop 执行；approve/reject 更新原 run；公网模式禁用共享 vault。
  - 文件：`src/safepatch/store/sqlite.py`、`src/safepatch/api/`、`src/safepatch/demo/workbench.py`、`src/safepatch/runtime.py`、相关测试。
  - 验证：应用重建后仍可 list/get run；反馈恢复场景含 failed check 与后续成功；HITL 场景可批准续跑或拒绝终止；public demo credential 接口返回 503；事件 payload 已脱敏。
  - 依赖：T81。
  - commit：`50c9527`。

- [x] T83 WebUI 交互与视觉收尾
  - 目标：建立可在桌面/移动端使用的 demo 控制台，自动展示 scenario、状态、时间线、审批、检查和 diff。
  - 文件：`src/safepatch/web/index.html`、`app.js`、`styles.css`、`tests/api/test_demo_workbench.py`。
  - 验证：WebUI contract 测试覆盖 scenario 控件、capability 状态、toast、审批自动填充与响应式断点契约；浏览器可通过 4 个场景全流程。
  - 依赖：T82。
  - commit：`50c9527`。
...
- [x] T84 安全、分发与文档收口
  ...
  - 验证：pytest 全部通过（104 passed, 1 skipped）；Docker build/run 通过；README 包含线上 URL、Docker、运行与安全配置。
  - 依赖：T83。
  - commit：`50c9527`、`b7255c6`。

- [x] T85 云端最终验收
  - 目标：提交并推送 GitHub，发布 Docker Hub 镜像，等待 CI 与 Render 部署完成，复测公网 WebUI 全流程。
  - 文件：`PLAN.md`、`AGENT_LOG.md`、`SPEC_PROCESS.md`。
  - 验证：GitHub Actions 最后一次 run 为 success；Render `/health` 返回 200；WebUI 4 场景可交互；公网 URL `https://safepatch-harness.onrender.com` 可访问。
  - 依赖：T84。
  - commit：`5ec5888`、`b7255c6`。
